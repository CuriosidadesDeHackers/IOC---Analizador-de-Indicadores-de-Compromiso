...
const StatsPanel: React.FC<StatsPanelProps> = React.memo(({ data, filteredCount }) => {
  const severityStats = React.useMemo(() => data.iocs.reduce((acc, ioc) => {
    acc[ioc.severity] = (acc[ioc.severity] || 0) + 1;
    return acc;
  }, {} as Record<string, number>), [data.iocs]);
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: AlertOctagon,
      logoType: 'ip', // Usando IP como logo para crítico
        ...
        >
          ...
          <div className="relative flex flex-col items-center text-center space-y-2 sm:space-y-3 lg:space-y-4">
            <div className="relative group/icon">
              ...
              <div className={`relative p-2 sm:p-3 lg:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-br ${stat.color} ${stat.hoverColor} flex-shrink-0 transform group-hover/icon:scale-110 transition-all duration-300 shadow-lg text-white`}>
                <IOCLogo type={stat.title.toLowerCase() === 'total' ? 'default' : stat.logoType || 'default'} className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7" />
              </div>
              {stat.value > 0 && (
