...
export class AdocParser {
        ...
        if (cells.length > 0 && tableHeaders.length > 0) {
          ...
          cells.forEach((cell) => {
            ...
            if (extractedIOCs.length > 0) {
              // Reducir ruido en consola para mejorar rendimiento
            }
            ...
            validIOCs.forEach(iocValue => {
              ...
              const ioc: IOC = {
                ...
                source: 'CuriosidadesDeHackers IOC Repository',
                dateAdded: new Date(),
                tags: tags,
                status: 'active',
                tableData: {
  ...
  private static buildDescriptionFromTable(tableData: Record<string, string>, section: string): string {
    ...
    return parts.length > 0 ? parts.join(' | ') : 'IOC extraído del repositorio de CuriosidadesDeHackers';
  }

  private static extractIOCsFromText(text: string): string[] {
    const iocs: string[] = [];

    // Remove AsciiDoc formatting and common defanging patterns
    let cleanText = text.replace(/\*([^*]+)\*/g, '$1') // bold
      .replace(/_([^_]+)_/g, '$1')   // italic
      .replace(/`([^`]+)`/g, '$1')   // code
      .replace(/\[([^\]]+)\]/g, (_, p1) => {
        // Handle [.] and [:] during AsciiDoc link stripping
        if (p1 === '.' || p1 === ':') return p1;
        return p1;
      })
    ...
    // Intensive Un-defanging
    cleanText = cleanText
      .replace(/\[\.\]|\(\.\)|\{\.\}|\s+\.\s+/g, '.')
      .replace(/\[:\]|\(:\)|\{:\}/g, ':')
      .replace(/\b[hH][xX]{2}[pP]\b/g, 'http')
      .replace(/\b[hH][xX]{2}[pP][sS]\b/g, 'https')
      .replace(/[\(]at[\)]|[\[]at[\]]|[\{]at[\}]/g, '@')
    ...
    if (domainMatches) {
      domainMatches.forEach(domain => {
        ...
        if (!lowerDomain.includes('example.') &&
          !lowerDomain.includes('test.') &&
          !lowerDomain.includes('localhost') &&
          !lowerDomain.includes('github.com') &&
          !lowerDomain.includes('asciidoc') &&
          !lowerDomain.match(/^\d+\.\d+$/) &&
          lowerDomain.includes('.') &&
          lowerDomain.length >= 4) {
          iocs.push(lowerDomain);
        }
      });
    }
